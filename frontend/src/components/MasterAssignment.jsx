  ...
  return (
    <Grid item xs={12}><Card sx={{ borderRadius: 4 }}><CardContent>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={3}><TextField fullWidth label="Trivia topic" value={cycle.topic} onChange={(event) => setCycle((current) => ({ ...current, topic: event.target.value }))} /></Grid>
        ...
        <Grid item xs={12} md={2}><Button fullWidth variant="contained" sx={{ height: '100%' }} onClick={onCreate} disabled={!cycle.master_username || !cycle.topic.trim()}>Add master</Button></Grid>
      </Grid>
